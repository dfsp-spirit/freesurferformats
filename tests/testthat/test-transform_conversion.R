...
test_that("An FSL matrix can be written, and only if it can be represented.", {
  ...
  world_tf <- transform2world(tf)
  expect_error(write.fs.transform(world_tf, mat_file, format = "fslmat"), "transform2voxel")
  expect_error(write.fs.transform.fslmat(world_tf, mat_file), "transform2voxel")
...
test_that("A transformation can be converted between voxel and world coordinates.", {
  ...
  back_to_voxel <- transform2voxel(world_tf)
  ...
  expect_equal(transform2world(world_tf), world_tf)
  expect_equal(transform2voxel(tf), tf)
  ...
  expect_error(transform2world(fsl_tf), "both volumes")
  expect_error(transform2world(fsl_tf, src = read.fs.volume(system.file("extdata", "brain.mgz", package = "freesurferformats", mustWork = TRUE), with_header = TRUE)), "both volumes")
  ...
  fsl_back <- transform2voxel(fsl_world)
  expect_equal(fsl_back$matrix, fsl_tf$matrix)
  expect_equal(fsl_back$voxel_base, 0L)
  expect_equal(transform2voxel(fsl_world)$matrix, fsl_tf$matrix)

  expect_error(transform2world("not a transform"))
  expect_error(transform2voxel("not a transform"))
  unlink(mat_file)
})
