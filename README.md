...
* CIFTI: The metadata (the XML part) of CIFTI v2 files is read by our own implementation: `read.cifti.header()` parses the CIFTI-2 XML of a file into an `fs.cifti` object that describes the matrix dimensions (series, brainordinates, parcels, scalars or labels), the brain models with their surface vertex and volume voxel indices, the parcels, the label tables and the series information, and `cifti.structures()`, `cifti.parcels()`, `cifti.series.info()` and `cifti.label.table()` extract the common parts. This works for all nine standard CIFTI-2 file types (`.dscalar`, `.dtseries`, `.dlabel`, `.dconn`, `.pscalar`, `.ptseries`, `.pconn`, `.dpconn`, `.pdconn`), for surface and for volume brain models, and it does not depend on the `cifti` package. Reading the data *values* of morphometry data (`.dscalar.nii`), cortical parcellations (`.dlabel.nii`) and surface time series (`.dtseries.nii`) is supported based on the [cifti package by John Muschelli](https://CRAN.R-project.org/package=cifti) (the wrapper functions extract the data for a specific brain model and map it to the appropriate vertex indices of the surface based on the CIFTI metadata); this part is being replaced by a native implementation as well.
