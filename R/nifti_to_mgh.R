...
#'
#' @param reorient logical, whether to let \code{oro.nifti::readNIfTI} reorient the data array to a standard orientation while reading it from a file. Defaults to `FALSE`, see the note below. Only relevant if `filepath` is a path, it is ignored if a `nifti` instance is passed.
#'
#' @param ... extra parameters passed to \code{oro.nifti::readNIfTI}. Leave this alone unless you know what you are doing. Note that `reorient` is passed explicitly by this function, so it cannot be set here.
#'
#' @return an `fs.volume` instance. The `header` fields are computed from the NIFTI header. The `data` array is returned exactly as present in the input NIFTI instance, i.e., no values are changed in any way. It is in the raw NIFTI file storage order (first dimension fastest), which is also the order used by the MGH/MGZ format. If the NIFTI file contains `sform` or `qform` geometry information, the returned header contains a `vox2ras_matrix` entry in addition to the MGH header fields, just like the header returned by \code{\link[freesurferformats]{read.fs.mgh}}.
#'
...
read.fs.volume.nii <- function(filepath, flatten = FALSE, with_header = FALSE, drop_empty_dims = FALSE, do_rotate = FALSE, reorient = FALSE, ...) {
  ...
  if (requireNamespace("oro.nifti", quietly = TRUE)) {
    if (is.character(nifti_img)) {
      ...
      nifti_img <- oro.nifti::readNIfTI(nifti_img, reorient = reorient, ...)
    } else {
      ...
      if (isTRUE(nifti_img@reoriented)) {
