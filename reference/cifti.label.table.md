...
Get the label table of one label map (a `.dlabel` file can contain
several maps). The label table maps integer label keys to names and RGBA
colors, and is the CIFTI analogue of a FreeSurfer color lookup table.
...
- map:
  ...
  `cii$matrix$indices_maps` or the `map_name` field of the result to
  find out which one you need.

## Value

a data.frame with one row per label and the columns 'key' (integer, the
label key used in the data), 'red', 'green', 'blue', 'alpha' (numeric,
in range 0 to 1) and 'label' (character string, the label name), plus
'x', 'y', 'z' (numeric, the optional label coordinates; `NA` if not
present in the file). Returns `NULL` if the specified matrix dimension
does not contain label maps, or if the specified map has no label table.
...
cifti.label.table(cii, dim = 0L, map = 1L)
#>   key red green blue alpha    label  x  y  z
#> 1   0   1   1.0    1     0      ??? NA NA NA
#> 2   1   0   0.5    1     1 PARCEL_A NA NA NA
#> 3   2   0   0.5    1     1 PARCEL_B NA NA NA
#> 4   3   0   0.5    1     1 PARCEL_C NA NA NA
```
