...
A parcel axis describes the parcels of a parcellated matrix dimension
(the mapping of `.pscalar`, `.ptseries`, `.pconn` and the second
dimension of `.pdconn`). As for brain models, the index of a parcel is
...
- volume:
  ...
  for parcels that consist of volume voxels, or `NULL`.

## Value

a named list with the entries 'type', 'parcels', 'surfaces' and
