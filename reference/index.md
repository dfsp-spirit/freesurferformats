# Package index

## All functions

- [`annot.max.region.idx()`](https://dfsp-spirit.github.io/freesurferformats/reference/annot.max.region.idx.md)
  : Get max region index of an fs.annot instance.

- [`atlas.from.lut.and.csv()`](https://dfsp-spirit.github.io/freesurferformats/reference/atlas.from.lut.and.csv.md)
  : Construct a brain atlas from a colortable (LUT) file and a
  per-vertex label file.

- [`bvsmp()`](https://dfsp-spirit.github.io/freesurferformats/reference/bvsmp.md)
  : Create new bvsmp instance encoding morph data for Brainvoyager.

- [`cdata()`](https://dfsp-spirit.github.io/freesurferformats/reference/cdata.md)
  : Create CDATA element string from string.

- [`closest.vert.to.point()`](https://dfsp-spirit.github.io/freesurferformats/reference/closest.vert.to.point.md)
  : Find vertex index closest to given query coordinate using Euclidean
  distance.

- [`colortable.from.annot()`](https://dfsp-spirit.github.io/freesurferformats/reference/colortable.from.annot.md)
  : Extract color lookup table (LUT) from annotation.

- [`delete_all_opt_data()`](https://dfsp-spirit.github.io/freesurferformats/reference/delete_all_opt_data.md)
  : Delete all data in the package cache.

- [`doapply.transform.mtx()`](https://dfsp-spirit.github.io/freesurferformats/reference/doapply.transform.mtx.md)
  : Apply a spatial transformation matrix to the given coordinates.

- [`download_opt_data()`](https://dfsp-spirit.github.io/freesurferformats/reference/download_opt_data.md)
  : Download optional data for the freesurferformats package.

- [`faces.quad.to.tris()`](https://dfsp-spirit.github.io/freesurferformats/reference/faces.quad.to.tris.md)
  : Convert quadrangular faces or polygons to triangular ones.

- [`faces.tris.to.quad()`](https://dfsp-spirit.github.io/freesurferformats/reference/faces.tris.to.quad.md)
  : Convert tris faces to quad faces by simple merging.

- [`flip2D()`](https://dfsp-spirit.github.io/freesurferformats/reference/flip2D.md)
  : Flip a 2D matrix.

- [`flip3D()`](https://dfsp-spirit.github.io/freesurferformats/reference/flip3D.md)
  : Flip a 3D array along an axis.

- [`fs.get.morph.file.ext.for.format()`](https://dfsp-spirit.github.io/freesurferformats/reference/fs.get.morph.file.ext.for.format.md)
  : Determine morphometry file extension from format

- [`fs.get.morph.file.format.from.filename()`](https://dfsp-spirit.github.io/freesurferformats/reference/fs.get.morph.file.format.from.filename.md)
  : Determine morphometry file format from filename

- [`fs.patch()`](https://dfsp-spirit.github.io/freesurferformats/reference/fs.patch.md)
  : Constructor for fs.patch

- [`fs.surface.to.tmesh3d()`](https://dfsp-spirit.github.io/freesurferformats/reference/fs.surface.to.tmesh3d.md)
  : Get an rgl tmesh3d instance from a brain surface mesh.

- [`get_opt_data_filepath()`](https://dfsp-spirit.github.io/freesurferformats/reference/get_opt_data_filepath.md)
  : Access a single file from the package cache by its file name.

- [`gifti_writer()`](https://dfsp-spirit.github.io/freesurferformats/reference/gifti_writer.md)
  : Write data to a gifti file.

- [`gifti_xml()`](https://dfsp-spirit.github.io/freesurferformats/reference/gifti_xml.md)
  : Get GIFTI XML representation of data.

- [`gifti_xml_add_global_metadata()`](https://dfsp-spirit.github.io/freesurferformats/reference/gifti_xml_add_global_metadata.md)
  : Add metadata to GIFTI XML tree.

- [`gifti_xml_write()`](https://dfsp-spirit.github.io/freesurferformats/reference/gifti_xml_write.md)
  : Write XML tree to a gifti file.

- [`giftixml_add_labeltable_from_annot()`](https://dfsp-spirit.github.io/freesurferformats/reference/giftixml_add_labeltable_from_annot.md)
  : Add a label tabel from an annotation to a GIFTI XML tree.

- [`is.bvsmp()`](https://dfsp-spirit.github.io/freesurferformats/reference/is.bvsmp.md)
  : Check whether object is a bvsmp instance.

- [`is.fs.annot()`](https://dfsp-spirit.github.io/freesurferformats/reference/is.fs.annot.md)
  : Check whether object is an fs.annot

- [`is.fs.label()`](https://dfsp-spirit.github.io/freesurferformats/reference/is.fs.label.md)
  : Check whether object is an fs.label

- [`is.fs.surface()`](https://dfsp-spirit.github.io/freesurferformats/reference/is.fs.surface.md)
  : Check whether object is an fs.surface

- [`is.fs.volume()`](https://dfsp-spirit.github.io/freesurferformats/reference/is.fs.volume.md)
  : Check whether object is an fs.volume

- [`is.mghheader()`](https://dfsp-spirit.github.io/freesurferformats/reference/is.mghheader.md)
  : Check whether object is an mghheader

- [`list_opt_data()`](https://dfsp-spirit.github.io/freesurferformats/reference/list_opt_data.md)
  : Get file names available in package cache.

- [`mghheader.centervoxelRAS.from.firstvoxelRAS()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.centervoxelRAS.from.firstvoxelRAS.md)
  : Compute RAS coords of center voxel.

- [`mghheader.crs.orientation()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.crs.orientation.md)
  : Compute MGH volume orientation string.

- [`mghheader.is.conformed()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.is.conformed.md)
  : Determine whether an MGH volume is conformed.

- [`mghheader.is.ras.valid()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.is.ras.valid.md)
  : Check whether header contains valid ras information

- [`mghheader.primary.slice.direction()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.primary.slice.direction.md)
  : Compute MGH primary slice direction

- [`mghheader.ras2vox()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.ras2vox.md)
  : Compute ras2vox matrix from basic MGH header fields.

- [`mghheader.ras2vox.tkreg()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.ras2vox.tkreg.md)
  : Compute ras2vox-tkreg matrix from basic MGH header fields.

- [`mghheader.scanner2tkreg()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.scanner2tkreg.md)
  : Compute scanner-RAS 2 tkreg-RAS matrix from basic MGH header fields.

- [`mghheader.tkreg2scanner()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.tkreg2scanner.md)
  : Compute tkreg-RAS to scanner-RAS matrix from basic MGH header
  fields.

- [`mghheader.update.from.vox2ras()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.update.from.vox2ras.md)
  : Update mghheader fields from vox2ras matrix.

- [`mghheader.vox2ras()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.vox2ras.md)
  : Compute vox2ras matrix from basic MGH header fields.

- [`mghheader.vox2ras.tkreg()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.vox2ras.tkreg.md)
  : Compute vox2ras-tkreg matrix from basic MGH header fields.

- [`mghheader.vox2vox()`](https://dfsp-spirit.github.io/freesurferformats/reference/mghheader.vox2vox.md)
  : Compute vox2vox matrix between two volumes.

- [`mni152reg()`](https://dfsp-spirit.github.io/freesurferformats/reference/mni152reg.md)
  : Get fsaverage (MNI305) to MNI152 transformation matrix.

- [`ni1header.for.data()`](https://dfsp-spirit.github.io/freesurferformats/reference/ni1header.for.data.md)
  : Create NIFTI v1 header suitable for given data.

- [`ni1header.template()`](https://dfsp-spirit.github.io/freesurferformats/reference/ni1header.template.md)
  : Create a template NIFTI v1 header. You will have to adapt it for
  your use case.

- [`ni2header.for.data()`](https://dfsp-spirit.github.io/freesurferformats/reference/ni2header.for.data.md)
  : Create NIFTI v2 header suitable for given data.

- [`ni2header.template()`](https://dfsp-spirit.github.io/freesurferformats/reference/ni2header.template.md)
  : Create a template NIFTI v2 header. You will have to adapt it for
  your use case.

- [`nifti.datadim.from.dimfield()`](https://dfsp-spirit.github.io/freesurferformats/reference/nifti.datadim.from.dimfield.md)
  : Compute data dimensions from the 'dim' field of the NIFTI (v1 or v2)
  header.

- [`nifti.datadim.to.dimfield()`](https://dfsp-spirit.github.io/freesurferformats/reference/nifti.datadim.to.dimfield.md)
  : Compute NIFTI dim field for data dimension.

- [`nifti.file.uses.fshack()`](https://dfsp-spirit.github.io/freesurferformats/reference/nifti.file.uses.fshack.md)
  : Determine whether a NIFTI file uses the FreeSurfer hack.

- [`nifti.file.version()`](https://dfsp-spirit.github.io/freesurferformats/reference/nifti.file.version.md)
  : Determine NIFTI file version information and whether file is a NIFTI
  file.

- [`nifti.header.check()`](https://dfsp-spirit.github.io/freesurferformats/reference/nifti.header.check.md)
  : Perform basic sanity checks on NIFTI header data. These are in no
  way meant to be exhaustive.

- [`nii1header.for.mgh()`](https://dfsp-spirit.github.io/freesurferformats/reference/nii1header.for.mgh.md)
  : Create a NIFTI v1 header from the header information contained in an
  fs.volume instance.

- [`print(`*`<fs.annot>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/print.fs.annot.md)
  : Print description of a brain atlas or annotation.

- [`print(`*`<fs.label>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/print.fs.label.md)
  : Print description of a brain surface label.

- [`print(`*`<fs.patch>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/print.fs.patch.md)
  : Print description of a brain surface patch.

- [`print(`*`<fs.surface>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/print.fs.surface.md)
  : Print description of a brain surface.

- [`print(`*`<fs.volume>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/print.fs.volume.md)
  : Print description of a brain volume.

- [`ras.to.surfaceras()`](https://dfsp-spirit.github.io/freesurferformats/reference/ras.to.surfaceras.md)
  : Translate RAS coordinates, as used in volumes by applying vox2ras,
  to surface RAS.

- [`ras.to.talairachras()`](https://dfsp-spirit.github.io/freesurferformats/reference/ras.to.talairachras.md)
  : Compute MNI talairach coordinates from RAS coords.

- [`read.dti.tck()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.dti.tck.md)
  : Read DTI tracking data from file in MRtrix 'TCK' format.

- [`read.dti.trk()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.dti.trk.md)
  : Read fiber tracks from Diffusion Toolkit in trk format.

- [`read.dti.tsf()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.dti.tsf.md)
  : Read DTI tracking per-coord data from file in MRtrix 'TSF' format.

- [`read.fs.annot()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.annot.md)
  : Read file in FreeSurfer annotation format

- [`read.fs.annot.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.annot.gii.md)
  : Read an annotation or label in GIFTI format.

- [`read.fs.colortable()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.colortable.md)
  : Read colortable file in FreeSurfer ASCII LUT format.

- [`read.fs.curv()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.curv.md)
  : Read file in FreeSurfer curv format

- [`read.fs.gca()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.gca.md)
  : Read FreeSurfer GCA file.

- [`read.fs.label()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.label.md)
  : Read a label file.

- [`read.fs.label.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.label.gii.md)
  : Read a label from a GIFTI label/annotation file.

- [`read.fs.label.native()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.label.native.md)
  : Read file in FreeSurfer label format

- [`read.fs.mgh()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.mgh.md)
  : Read file in FreeSurfer MGH or MGZ format

- [`read.fs.morph()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.md)
  : Read morphometry data file in any FreeSurfer format.

- [`read.fs.morph.asc()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.asc.md)
  : Read morphometry data from ASCII curv format file

- [`read.fs.morph.bvsmp()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.bvsmp.md)
  : Read Brainvoyager vertex-wise statistical surface data from SMP
  file.

- [`read.fs.morph.cifti()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.cifti.md)
  : Read surface morphometry data from CIFTI dscalar files.

- [`read.fs.morph.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.gii.md)
  : Read morphometry data file in GIFTI format.

- [`read.fs.morph.ni1()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.ni1.md)
  : Read morphometry data from FreeSurfer NIFTI v1 format files.

- [`read.fs.morph.ni2()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.ni2.md)
  : Read morphometry data from FreeSurfer NIFTI v2 format files.

- [`read.fs.morph.nii()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.nii.md)
  : Read morphometry data from FreeSurfer NIFTI format files, determine
  NIFTI version automatically.

- [`read.fs.morph.txt()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.morph.txt.md)
  : Read morphometry data from plain text file

- [`read.fs.parcellation.cifti()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.parcellation.cifti.md)
  : Read surface parcellation data from CIFTI dlabel files.

- [`read.fs.patch()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.patch.md)
  : Read FreeSurfer binary or ASCII patch file.

- [`read.fs.patch.asc()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.patch.asc.md)
  : Read FreeSurfer ASCII format patch.

- [`read.fs.series.cifti()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.series.cifti.md)
  : Read surface time series data from CIFTI dtseries files.

- [`read.fs.surface()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.md)
  : Read file in FreeSurfer surface format or various mesh formats.

- [`read.fs.surface.asc()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.asc.md)
  : Read FreeSurfer ASCII format surface.

- [`read.fs.surface.bvsrf()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.bvsrf.md)
  : Read Brainvoyager srf format (.srf) mesh as surface.

- [`read.fs.surface.byu()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.byu.md)
  : Read mesh in BYU format.

- [`read.fs.surface.geo()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.geo.md)
  : Read GEO format mesh as surface.

- [`read.fs.surface.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.gii.md)
  : Read GIFTI format mesh as surface.

- [`read.fs.surface.ico()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.ico.md)
  : Read ICO format mesh as surface.

- [`read.fs.surface.mz3()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.mz3.md)
  : Read surface mesh in mz3 format, used by Surf-Ice.

- [`read.fs.surface.obj()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.obj.md)
  : Read OBJ format mesh as surface.

- [`read.fs.surface.off()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.off.md)
  : Read Object File Format (OFF) mesh as surface.

- [`read.fs.surface.ply()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.ply.md)
  : Read Stanford PLY format mesh as surface.

- [`read.fs.surface.stl()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.stl.md)
  : Read mesh in STL format, auto-detecting ASCII versus binary format
  version.

- [`read.fs.surface.stl.bin()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.stl.bin.md)
  : Read surface mesh in STL binary format.

- [`read.fs.surface.vtk()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.surface.vtk.md)
  : Read VTK ASCII format mesh as surface.

- [`read.fs.transform()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.transform.md)
  : Load transformation matrix from a file.

- [`read.fs.transform.dat()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.transform.dat.md)
  : Load transformation matrix from a tkregister dat file.

- [`read.fs.transform.lta()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.transform.lta.md)
  : Load transformation matrix from a FreeSurfer linear transform array
  (LTA) file.

- [`read.fs.transform.xfm()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.transform.xfm.md)
  : Load transformation matrix from an XFM file.

- [`read.fs.volume()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.volume.md)
  : Read volume file in MGH, MGZ or NIFTI format

- [`read.fs.volume.nii()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.volume.nii.md)
  :

  Turn a 3D or 4D `oro.nifti` instance into an `fs.volume` instance with
  complete header.

- [`read.fs.weight()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.fs.weight.md)
  : Read file in FreeSurfer weight or w format

- [`read.mesh.brainvoyager()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.mesh.brainvoyager.md)
  : Read Brainvoyager srf format (.srf) mesh.

- [`read.nifti1.data()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.nifti1.data.md)
  : Read raw NIFTI v1 data from file (which may contain the FreeSurfer
  hack).

- [`read.nifti1.header()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.nifti1.header.md)
  : Read NIFTI v1 header from file (which may contain the FreeSurfer
  hack).

- [`read.nifti2.data()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.nifti2.data.md)
  : Read raw data from NIFTI v2 file.

- [`read.nifti2.header()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.nifti2.header.md)
  : Read NIFTI v2 header from file.

- [`read.smp.brainvoyager()`](https://dfsp-spirit.github.io/freesurferformats/reference/read.smp.brainvoyager.md)
  : Read Brainvoyager statistical surface results from SMP file.

- [`read_nisurface()`](https://dfsp-spirit.github.io/freesurferformats/reference/read_nisurface.md)
  : Read a surface, based on the file path without extension.

- [`read_nisurfacefile()`](https://dfsp-spirit.github.io/freesurferformats/reference/read_nisurfacefile.md)
  : S3 method to read a neuroimaging surface file.

- [`read_nisurfacefile(`*`<fsascii>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/read_nisurfacefile.fsascii.md)
  : Read a FreeSurfer ASCII surface file.

- [`read_nisurfacefile(`*`<fsnative>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/read_nisurfacefile.fsnative.md)
  : Read a FreeSurfer ASCII surface file.

- [`read_nisurfacefile(`*`<gifti>`*`)`](https://dfsp-spirit.github.io/freesurferformats/reference/read_nisurfacefile.gifti.md)
  : Read a gifti file as a surface.

- [`readable.files()`](https://dfsp-spirit.github.io/freesurferformats/reference/readable.files.md)
  : Find files with the given base name and extensions that exist.

- [`rotate2D()`](https://dfsp-spirit.github.io/freesurferformats/reference/rotate2D.md)
  : Rotate a 2D matrix in 90 degree steps.

- [`rotate3D()`](https://dfsp-spirit.github.io/freesurferformats/reference/rotate3D.md)
  : Rotate a 3D array in 90 degree steps.

- [`sm0to1()`](https://dfsp-spirit.github.io/freesurferformats/reference/sm0to1.md)
  : Adapt spatial transformation matrix for 1-based indices.

- [`sm1to0()`](https://dfsp-spirit.github.io/freesurferformats/reference/sm1to0.md)
  : Adapt spatial transformation matrix for 0-based indices.

- [`surfaceras.to.ras()`](https://dfsp-spirit.github.io/freesurferformats/reference/surfaceras.to.ras.md)
  : Translate surface RAS coordinates, as used in surface vertices and
  surface labels, to volume RAS.

- [`surfaceras.to.talairach()`](https://dfsp-spirit.github.io/freesurferformats/reference/surfaceras.to.talairach.md)
  : Compute Talairach RAS for surface RAS (e.g., vertex coordinates).

- [`talairachras.to.ras()`](https://dfsp-spirit.github.io/freesurferformats/reference/talairachras.to.ras.md)
  : Compute MNI talairach coordinates from RAS coords.

- [`vertex.euclid.dist()`](https://dfsp-spirit.github.io/freesurferformats/reference/vertex.euclid.dist.md)
  : Compute Euclidean distance between two vertices v1 and v2.

- [`vertexdists.to.point()`](https://dfsp-spirit.github.io/freesurferformats/reference/vertexdists.to.point.md)
  : Compute Euclidean distance from all mesh vertices to given point.

- [`write.atlas.to.lut.and.csv()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.atlas.to.lut.and.csv.md)
  : Write a brain atlas to a colortable (LUT) file and a per-vertex
  label file.

- [`write.fs.annot()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.annot.md)
  : Write annotation to binary file.

- [`write.fs.annot.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.annot.gii.md)
  : Write annotation to GIFTI file.

- [`write.fs.colortable()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.colortable.md)
  : Write colortable file in FreeSurfer ASCII LUT format.

- [`write.fs.curv()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.curv.md)
  : Write file in FreeSurfer curv format

- [`write.fs.label()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.label.md)
  : Write vertex indices to file in FreeSurfer label format

- [`write.fs.label.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.label.gii.md)
  : Write a binary surface label in GIFTI format.

- [`write.fs.mgh()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.mgh.md)
  : Write file in FreeSurfer MGH or MGZ format

- [`write.fs.morph()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.morph.md)
  : Write morphometry data in a format derived from the given file name.

- [`write.fs.morph.asc()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.morph.asc.md)
  : Write file in FreeSurfer ASCII curv format

- [`write.fs.morph.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.morph.gii.md)
  : Write morphometry data in GIFTI format.

- [`write.fs.morph.ni1()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.morph.ni1.md)
  : Write morphometry data in NIFTI v1 format.

- [`write.fs.morph.ni2()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.morph.ni2.md)
  : Write morphometry data in NIFTI v2 format.

- [`write.fs.morph.smp()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.morph.smp.md)
  : Write morphometry data in Brainvoyager SMP format.

- [`write.fs.morph.txt()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.morph.txt.md)
  : Write curv data to file in simple text format

- [`write.fs.patch()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.patch.md)
  : Write a surface patch

- [`write.fs.surface()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.md)
  : Write mesh to file in FreeSurfer binary surface format

- [`write.fs.surface.asc()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.asc.md)
  : Write mesh to file in FreeSurfer ASCII surface format

- [`write.fs.surface.bvsrf()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.bvsrf.md)
  : Write surface to Brainvoyager SRF file.

- [`write.fs.surface.byu()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.byu.md)
  : Write mesh to file in BYU ASCII format.

- [`write.fs.surface.gii()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.gii.md)
  : Write mesh to file in GIFTI surface format

- [`write.fs.surface.mz3()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.mz3.md)
  : Write mesh to file in mz3 binary format.

- [`write.fs.surface.obj()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.obj.md)
  : Write mesh to file in Wavefront object (.obj) format

- [`write.fs.surface.off()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.off.md)
  : Write mesh to file in Object File Format (.off)

- [`write.fs.surface.ply()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.ply.md)
  : Write mesh to file in PLY format (.ply)

- [`write.fs.surface.ply2()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.ply2.md)
  : Write mesh to file in PLY2 File Format (.ply2)

- [`write.fs.surface.vtk()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.surface.vtk.md)
  : Write mesh to file in VTK ASCII format

- [`write.fs.volume()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.volume.md)
  : Write an fs.volume instance to a file in MGH, MGZ or NIFTI v1
  format.

- [`write.fs.weight()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.weight.md)
  : Write file in FreeSurfer weight format

- [`write.fs.weight.asc()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.fs.weight.asc.md)
  : Write file in FreeSurfer weight ASCII format

- [`write.nifti1()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.nifti1.md)
  : Write header and data to a file in NIFTI v1 format.

- [`write.nifti2()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.nifti2.md)
  : Write header and data to a file in NIFTI v2 format.

- [`write.smp.brainvoyager()`](https://dfsp-spirit.github.io/freesurferformats/reference/write.smp.brainvoyager.md)
  : Write a brainvoyager SMP file.

- [`xml_node_gifti_coordtransform()`](https://dfsp-spirit.github.io/freesurferformats/reference/xml_node_gifti_coordtransform.md)
  : Create XML GIFTI CoordinateSystemTransformMatrix node.
