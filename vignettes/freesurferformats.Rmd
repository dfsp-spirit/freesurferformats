...
```{r, eval = FALSE}
    ...
    header$ras_good_flag   # int, 0 or 1. Whether the file contains a valid ras2vox matrix and ras_xform (see header$ras2vox_matrix below)
...
If your MGH/MGZ file contains valid information on the ras2vox matrix and/or acquisition parameters (`mr_params`), you can access them like this for the mr params:
...
And like this for the `ras2vox_matrix`:

```{r}
    if(header$ras_good_flag) {
        print(header$ras2vox_matrix);
    }
```
...
```{r}
    ...
    hist(thickness_native, main="lh cortical thickness", xlab="Cortical thickness [mm]", ylab="Vertex count")
```  
...
```{r}
    ...
    hist(curv_standard, main="lh std curvature", xlab="Mean Curvature [mm^-1]", ylab="Vertex count")
